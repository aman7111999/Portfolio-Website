-- Prepared for review only. Apply after the updated experience copy is approved.

update public.site_settings
set
  tagline = 'Product designer with 4.5+ years building fintech products at Motilal Oswal and Trinkerr.',
  bio = 'Product designer with 4.5+ years building fintech products at Motilal Oswal and Trinkerr. I enjoy turning complex investment journeys and dense information into experiences people can understand and act on. I work closely with product and engineering, from figuring out the problem to seeing the final experience go live.'
where id = 1;
...
set
  ...
  start_date = 'Aug 2025',
  end_date = null,
  description = 'Designing Riise, Motilal Oswal''s investment platform for 1.6 million active clients across Stocks, F&O, Mutual Funds, US Stocks, Algo, AIF and PMS.',
  highlights = array[
  ...
  published = true,
  updated_at = now()
where id = '6f4477c7-b0ac-493f-9536-2950ccb0542f';
...
set
  ...
  start_date = 'Jan 2022',
  end_date = 'Apr 2025',
  description = 'Product Designer (Feb 2023 - Apr 2025), promoted from Associate Product Designer (Jan 2022 - Feb 2023). Worked across portfolio tools, investment discovery and the TIQS product.',
  highlights = array[
  ...
  published = true,
  updated_at = now()
where id = '4ff89901-8bf1-4875-9dbc-77857d79172e';

-- The updated resume presents the Trinkerr promotion as one combined experience.
update public.experience
set published = false, updated_at = now()
where id = 'a881f8ef-ca97-44cd-bcdb-1db5ca0c8173';

update public.content_blocks
set data = jsonb_set(
  jsonb_set(data, '{heading_line1}', to_jsonb('4.5+ years building'::text), true),
  '{heading_line2}', to_jsonb('fintech products.'::text), true
)
...
set data = jsonb_set(
  data,
  '{subline}',
  to_jsonb('I enjoy turning complex investment journeys and dense information into experiences people can understand and act on. I work closely with product and engineering, from figuring out the problem to seeing the final experience go live.'::text),
  true
)
where key = 'hero';
